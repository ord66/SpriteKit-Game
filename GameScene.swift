...
//  Travel Win
//
//  Created by osman resat dogrul on 28.03.2020.
...
/// which object types collide or have contact wiith each other
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    // Enum for y-position spam points for bricks
    ...
    // THis may invrease as the user progresses in the game
    ...
    override func didMove(to view: SKView) {
       ...
       let yMid = frame.midY
        ...
        menuLayer.display(message: "Stay Home Save Lives - Tap to play", score: nil)
        addChild(menuLayer)
    ...
    func spawnBrick(atPosition position: CGPoint) -> SKSpriteNode {
        // Create a brick sprite and add it to the secene
    ...
    func updateBricks(withScrollAmount currentScrollAmount: CGFloat) {
        ...
        while farthestRightBrickX < frame.width {
            ...
            // Every mow then, leave a gap the player must jump over
    ...
    func updateScore(withCurrentTime currentTime: TimeInterval) {
        // The player`s score increases the lonher they survive
        ...
        if elapsedTime > 1.0 {
            ...
            // Rset the lastScoreUpdateTime to the current time
    ...
    @objc func handleTap(tapGesture: UITapGestureRecognizer) {
        ...
        if  skater.isOnGround {
            skater.physicsBody?.applyImpulse(CGVector(dx: 0.0, dy: 660.0))
            
            run(SKAction.playSoundFileNamed("jump.wav", waitForCompletion: false))
            
            //var gemCaptureSound: SKAction { return SKAction.playSoundFileNamed("jump.wav", waitForCompletion: false)}
            }
        }
         else {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contact.bodyA.categoryBitMask == PhysicsCategory.skater && contact.bodyB.categoryBitMask == PhysicsCategory.brick {
            ...
            if let gem = contact.bodyB.node as? SKSpriteNode {
                ...
                //var gemCaptureSound: SKAction { return SKAction.playSoundFileNamed("gem.wav", waitForCompletion: false)}
